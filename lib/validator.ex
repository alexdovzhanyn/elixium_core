defmodule UltraDark.Validator do
  ...
  defp valid_index(index, prev_index) when index <= prev_index, do: {:error, {:invalid_index, prev_index, index}}
  ...
  defp valid_prev_hash(prev_hash, last_block_hash) when prev_hash != last_block_hash, do: {:error, {:wrong_hash, {:doesnt_match_last, prev_hash, last_block_hash}}}
  ...
  defp valid_hash(%{index: index, previous_hash: previous_hash, timestamp: timestamp, nonce: nonce, hash: hash, merkle_root: merkle_root, difficulty: difficulty}) do
    with :ok <- compare_hash({index, previous_hash, timestamp, nonce, merkle_root}, hash),
       :ok <- fn -> if Block.hash_beat_target?(%{hash: hash, difficulty: difficulty}), do: :ok, else: {:error, {:wrong_hash, {:too_low, hash, difficulty}}} end
    do :ok
	  else err -> err
	  end
  end

  @spec compare_hash({number, String.t, String.t, number, String.t}, String.t) :: :ok | {:error, {:wrong_hash, {:doesnt_match_provided, String.t, String.t}}}
  defp compare_hash({index, previous_hash, timestamp, nonce, merkle_root}, hash) do
    computed = [Integer.to_string(index), previous_hash, timestamp, Integer.to_string(nonce), merkle_root] |> Utilities.sha3_base16
    if computed == hash do
      :ok
    else
